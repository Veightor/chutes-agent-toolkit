...
| `chutes_get_evidence` | `GET /chutes/{id}/evidence?nonce=<64 hex>` | **[BETA]** (needs a chute_id; nonce required by API, auto-generated by the tool) | "Show me TEE evidence for this chute" |
...
| `chutes_teeify` | `PUT /chutes/{id}/teeify` | **Endpoint removed upstream** — not present in `api.chutes.ai/openapi.json` as of 2026-06-11; the call will fail. TEE is now a deploy-time switch (`tee=True` on the SDK chute templates). Tool kept registered with a deprecation notice |
